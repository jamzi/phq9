...
export function completeLocalQuestionnaire(run, answers, difficulty) {
  return [
    ...answers.map((value, index) => [
      'click',
      `label[for="q${index + 1}-${value}"]`,
    ]),
    ['click', `label[for="difficulty-${difficulty}"]`],
    ['click', '[data-testid="submit-button"]'],
  ]
}

export function selectLocalItem9(run, value) {
  return [['click', `label[for="q9-${value}"]`]]
}
